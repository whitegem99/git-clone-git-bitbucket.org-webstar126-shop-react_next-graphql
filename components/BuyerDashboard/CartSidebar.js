...
import CartSummary from '../VideoPopup/cartSummary'
import CheckoutCard from '../VideoPopup/checkoutCard'
import ApplyCredits from '../VideoPopup/applyCredits'
import AddNotes from '../VideoPopup/addNotes'
    ...
    <>
      {isCardShow ? (
        <>
          ...
          {buyerPaymentMethod?.data?.findBuyerPaymentMethods?.paymentMethods
            ?.filter(card => card.id === paymentMethodId)
            ?.map(itm => (
          ...
          <div className="d-flex cartButton justify-content-between align-items-center mt-3 w-50">
            <Button
              size="lg"
              color="dark"
        ...
        <>
          ...
          <form onSubmit={handleStripe} className="w-50">
            <CardElement
              options={{
                hidePostalCode: true
              }}
...
const CartSidebar = ({ isCartOpen, setCartOpen, cartDetailsData }) => {
  ...
  const removeFromCartHandler = async id => {
    ...
    try {
      const {
        data: {
          orderRemoveProduct: { orders, errors }
        }
  ...
  const handleSameBillingAddressCheck = (e, props) => {
    if (e.target.checked) {
      props.setValues({
  ...
  return (
    <div
      className={
        isCartOpen
          ? 'sidebar-cart active-cart box-shadow shadow text-black py-4 px-4'
          : 'sidebar-cart'
      }
    >
      <div className="checkout">
        ...
        {step ===
        (cartStateData[0]?.company?.credits === 'undefined' ||
        cartStateData[0]?.company?.credits > 0
          ? 8
          : 7) ? (
          <>
            ...
            >
              ...
              <div className="d-block">
                <Link href="/buyers/dashboard/orders">
                  <Button
                    onClick={async () => {
                      setCartOpen(false)
                      await cartData.refetch()
                      ...
                      <>
                        <div className="cards-container">
                          {checkoutCartOrders.map(
                            order =>
                              order.cartProductsGrid.length > 0 && (
                                <CheckoutCard
                                  key={order.id}
          ...
          <>
            ...
            >
              ...
              <Button
                onClick={() => setCartOpen(false)}
                className="addTocart-btn m-auto"
              >
                Continue Shopping
              </Button>
            </div>
